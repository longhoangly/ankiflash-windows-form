...
Changed from v1: Merge initUi method into __init__ object
Changed from v2: Check model base on name, before importing
Changed from v3: Fix bug note type, fix size main window
Changed from v4: Remove combo box languages, add read option from file
Changed from v5: Only copy necessary files
Changed from v6: Changed file name to Language.txt, import csv and increase max field size
Changed from v7: Add more languages
"""
...
class AutoImportWindows(QtGui.QWidget):
    def __init__(self):
        ...
        self.setWindowTitle('Auto Import Flashcards')
    ...
    def btnImportClicked(self):
        ...
        activeOpt[0] = activeOpt[0].replace('\r', '')
        print "activecom[0]: " + activeOpt[0]
        if activeOpt[0] in ["[en] Vietnamese", "[en] English"]:
...
action = QAction("Auto Import Flashcards", mw)
